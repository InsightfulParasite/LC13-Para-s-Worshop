...
/mob/living/simple_animal/hostile/lillibag/Life()
	...
	if((LAZYLEN(contents) >= 20) || resources >= 30)
		var/where_is_everyone = FALSE
		for(var/mob/living/simple_animal/hostile/lilliputian/L in followers)
			if(L.loc != src)
				L.behavior_mode = LILI_BEHAVIOR_MODE_RETURN
...
/mob/living/simple_animal/hostile/lilliputian/Life()
	...
	if(!.) // Dead
		return FALSE
	if(behavior_change_cooldown <= world.time && behavior_mode != LILI_BEHAVIOR_MODE_RETURN)
...
/mob/living/simple_animal/hostile/lilliputian/AttackingTarget()
	...
	if(behavior_mode == LILI_BEHAVIOR_MODE_RETURN &&  home && target == home)
		forceMove(home)
		can_act = FALSE
		return
...
/mob/living/simple_animal/hostile/lilliputian/proc/CheckAndCallBackup(mob/living/the_target)
	...
	//Randomize how loud they call for help.
	for(var/mob/living/simple_animal/hostile/lilliputian/L in view(rand(0,3), src))
		if(L.stat == DEAD)
			continue
		if(L.target)
			continue
